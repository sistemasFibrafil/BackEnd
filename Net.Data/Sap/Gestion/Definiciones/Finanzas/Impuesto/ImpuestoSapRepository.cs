...
namespace Net.Data.Sap
{
    public class ImpuestoSapRepository : RepositoryBase<ImpuestoSapEntity>, IImpuestoSapRepository
    {
        ...
        const string SP_GET_LIST_BY_FILTRO = DB_ESQUEMA + "GES_GetListImpuestoByFiltro";
        const string SP_GET_BY_SLPCODE = DB_ESQUEMA + "GES_GetImpuestoBySlpCode";


        public ImpuestoSapRepository(IConnectionSQL context, IConfiguration configuration)
            : base(context)
        {
        ...
        public async Task<ResultadoTransaccionEntity<ImpuestoSapEntity>> GetListByFiltro(FilterRequestEntity value)
        {
            var response = new List<ImpuestoSapEntity>();
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(_cnxSap))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(SP_GET_LIST_BY_FILTRO, conn))
                    {
                        ...
                        {
                            response = (List<ImpuestoSapEntity>)context.ConvertTo<ImpuestoSapEntity>(reader);
                        }
                    }
        ...
        public async Task<ResultadoTransaccionEntity<ImpuestoSapEntity>> GetBySplCode(FilterRequestEntity value)
        {
            var response = new ImpuestoSapEntity();
            var resultTransaccion = new ResultadoTransaccionEntity<ImpuestoSapEntity>();
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(_cnxSap))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(SP_GET_BY_SLPCODE, conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;
                        ...
                        using (var reader = await cmd.ExecuteReaderAsync())
                        {
                            response = context.Convert<ImpuestoSapEntity>(reader);
                        }
                    }
